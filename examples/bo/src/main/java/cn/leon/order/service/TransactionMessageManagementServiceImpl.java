...
import cn.leon.order.persistence.TransactionalMessageDao;
import cn.leon.order.sink.Sink;
import lombok.RequiredArgsConstructor;
import lombok.extern.slf4j.Slf4j;
import org.aspectj.weaver.loadtime.JRockitAgent;
...
public class TransactionMessageManagementServiceImpl implements ITransactionMessageManagementService {
...
//    private final RabbitTemplate rabbitTemplate;
    ...
    public void sendMessageSync(TransactionalMessage record, String content) {
        try {
//            streamTemplate.send(content);
            sink.output().send(MessageBuilder.withPayload(content).build());
